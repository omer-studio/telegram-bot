...
### 📊 **מערכת אבחון ביצועים:**
- `performance_monitor_advanced.py` — מדידת TTFT ו-TPS עבור אבחון צוואר בקבוק
- `test_performance_system.py` — בדיקות אוטומטיות למערכת הביצועים
...
## 📊 מערכת אבחון צוואר בקבוק

### 🎯 מטרת המערכת

מערכת זו נועדה לאבחן צוואר בקבוק בביצועי הבוט על ידי מדידה מדויקת של:

1. **TTFT (Time To First Token)** - זמן האחזור הראשוני
2. **TPS (Tokens Per Second)** - קצב התפוקה  
3. **ניתוח דפוסי ביצועים** - זיהוי בעיות ספציפיות

המטרה: לקבוע האם הבעיה העיקרית היא בזמן האחזור הראשוני או בקצב התפוקה, ולקבל המלצות מבוססות נתונים.

### 🔬 רכיבי המערכת

#### 1. מודול מדידת ביצועים (`performance_monitor_advanced.py`)
- **מה הוא עושה**: מודד TTFT ו-TPS עבור כל אינטראקציה
- **איך**: רושם זמנים בתחילת ובסוף כל תגובה של ה-AI
- **נתונים שנאספים**: זמני תגובה, מספרי טוקנים, מודלים בשימוש, שגיאות
- **יעד איסוף**: 50-100 אינטראקציות אמיתיות

#### 2. פקודות ניהול (ב-`secret_commands.py`)
- **#perf_status**: בדיקת מצב איסוף נתונים והתקדמות
- **#perf_analyze**: ניתוח מפורט וזיהוי צוואר בקבוק  
- **#perf_clear**: איפוס נתונים (לשימוש זהיר)

#### 3. שילוב עם המערכת (`gpt_a_handler.py`)
- **מדידה אוטומטית**: כל תגובה נמדדת ללא צורך בהתערבות
- **טיפול בשגיאות**: רישום כישלונים למטרות ניתוח
- **שקיפות**: המערכת לא משפיעה על חוויית המשתמש

### 📊 איך להשתמש במערכת

#### שלב 1: איסוף נתונים (בהתקדמות)
- המערכת כבר פעילה ואוספת נתונים מכל אינטראקציה
- **לעשות**: המשך להשתמש בבוט כרגיל
...
דוח דוגמה:
```
🔬 מצב איסוף נתוני ביצועים
📊 התקדמות: ██░░░░░░░░ 20.0%
📈 נתונים שנאספו: 20/100
🎯 סטטוס: ⏳ צריך עוד נתונים
```

#### שלב 3: ניתוח (כשיש מספיק נתונים)
```
#perf_analyze  
```
דוח דוגמה:
```
🔬 ניתוח ביצועים - אבחון צוואר בקבוק
🐌 בעיה עיקרית: ttft (זמן אחזור איטי)
⚡ TTFT ממוצע: 4.20 שניות
🚀 TPS ממוצע: 25.3 טוקנים/שנייה
💡 המלצה: מעבר למודל מהיר יותר
```

### 🎯 סוגי אבחונים אפשריים

#### 🐌 בעיית TTFT - זמן אחזור איטי
**תסמינים**:
- זמן אחזור > 3 שניות בממוצע
- המשתמש חווה "שתיקה" ראשונית ארוכה

**פתרונות**:
1. מעבר מ-Gemini 1.5 Pro ל-Gemini 1.5 Flash
2. קיצור הפרומפטים  
3. שימוש ב-caching
4. מודלים קטנים לשאלות פשוטות

#### 🚧 בעיית TPS - קצב יצירה איטי  
**תסמינים**:
- פחות מ-10 טוקנים לשנייה
- התגובה נכתבת לאט על המסך

**פתרונות**:
1. מעבר למודל מהיר יותר
2. בדיקת צוואר בקבוק ברשת/שרת
3. אופטימיזציה של פרמטרי מודל
4. שימוש ב-streaming

#### ⚠️ בעיה משולבת
**תסמינים**:
- גם TTFT גבוה וגם TPS נמוך
- חוויית משתמש איטית מאוד

**פתרונות**:
- שדרוג כולל למודל מהיר יותר
- בחינת תשתית המערכת
- אופטימיזציה מקיפה
...
```
data/
├── performance_metrics.jsonl    # נתונים גולמיים של כל מדידה
├── performance_summary.json    # ניתוח מסכם מפורט
└── (קבצי נתונים אחרים של הבוט)
```

### ⚠️ שיקולים חשובים

#### מגבלות מדויקות
- **TTFT**: מדוייק לרמת שניות (לא מילישניות)
- **TPS**: הערכה על בסיס אורך טקסט, לא ספירה אמיתית של טוקנים
- **השפעות חיצוניות**: עומס רשת, שרת, ומודל AI

#### המלצות לדיוק
- איסוף בזמנים שונים ביום
- דגימות מסוגי שאלות מגוונים  
- השוואה בין מודלים שונים
- מדידות תחת עומס נמוך וגבוה
...
- `data/performance_metrics.jsonl` - נתוני ביצועים
...
- `#perf_status` - מצב מערכת אבחון הביצועים
- `#perf_analyze` - ניתוח ביצועים ואבחון צוואר בקבוק
- `#perf_clear` - איפוס נתוני ביצועים
...
3. **"תגובות איטיות"** → השתמש במערכת אבחון הביצועים
4. **"מדידות לא מדויקות"** → בדוק שגיאות בלוגים
