...
def update_chat_history(chat_id: str, user_msg: str, bot_msg: str, **kwargs) -> bool:
    """
    🔥 **פונקציה מפושטת**: עדכון היסטוריה בסיסי בלבד
    ...
    ⚠️ רישום מפורט ב-interactions_log מתבצע רק ב-message_handler.py
    
    Args:
        chat_id: מזהה הצ'אט
        user_msg: הודעת המשתמש  
        bot_msg: הודעת הבוט
        **kwargs: פרמטרים נוספים (לא נעשה בהם שימוש לרישום מפורט)
