...
לוגיקה כללית:
1. כל הודעה נכנסת — קודם כל מחפשים את ה-chat_id בעמודה 1 של גיליון 1 (access_codes).
2. אם נמצא — המשתמש כבר רשום, תהליך רגיל.
3. אם לא נמצא — בודקים אם ה-chat_id בעמודה 1 של user_states.
4. אם גם שם לא נמצא — זו פנייה ראשונה, מוסיפים שורה חדשה ל-user_states עם code_try=0.
5. שאר ההיגיון: ניהול הרשאות, רישום משתמש, אישור תנאים, מענה, עדכון לוגים, חישוב עלויות, שמירת היסטוריה.

הקובץ אחראי לתיאום בין כל המודולים, תיעוד לוגים, בקרה, טיפול בשגיאות, ושמירה על סדר פעולות תקין.
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    הפונקציה שמטפלת בכל הודעה:
    1. מזהה אם המשתמש חדש (קריאה ל-ensure_user_state_row).
    2. בודקת הרשאות משתמש.
    3. מטפלת ברישום/אישור/מענה בהתאם למצב.
    4. כל שלב מתועד בלוג ופרינט.
    """
    ...
    # שלב 1: בדיקת משתמש חדש
    try:
        logging.info("[Onboarding] בודק האם המשתמש חדש...")
        print("[Onboarding] בודק האם המשתמש חדש...")
        created = ensure_user_state_row(
            context.bot_data["sheet"],  # גיליון 1 (access_codes)
            context.bot_data["sheet_states"],  # גיליון user_states
            chat_id
        )
        if created:
            logging.info(f"[Onboarding] משתמש חדש - נוסף ל-user_states (code_try=0): {chat_id}")
            print(f"[Onboarding] משתמש חדש - נוסף ל-user_states (code_try=0): {chat_id}")
            await update.message.reply_text("היי מלך! 👑 אני רואה שזו הפעם הראשונה שלך כאן...")
